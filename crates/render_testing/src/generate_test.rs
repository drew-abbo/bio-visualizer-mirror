...
impl RenderTests {
    ...
    pub fn test_invert(&mut self) -> Result<wgpu::TextureView, Box<dyn std::error::Error>> {
        ...
        graph.set_input_value(
            ...
            InputValue::File(PathBuf::from(
                "C:\\Users\\Zach\\Downloads\\integrate_jobtread.png",
            )),
        )?;

        // Connect image output to invert input
        graph.connect(image, "output".to_string(), invert, "input".to_string())?;

        // Execute the graph - executor handles everything!
        let result =
            self.executor
                .execute(&graph, &self.node_library, &self.device, &self.queue)?;

        // Get the output frame
        let output = result.outputs.get("output").ok_or("No output found")?;
    ...
    pub fn test_brightness(&mut self) -> Result<wgpu::TextureView, Box<dyn std::error::Error>> {
        ...
        graph.set_input_value(brightness, "brightness".to_string(), InputValue::Float(2.0))?;
    ...
    pub fn test_effect_chain(&mut self) -> Result<wgpu::TextureView, Box<dyn std::error::Error>> {
        ...
        // Set parameters
        graph.set_input_value(brightness, "brightness".to_string(), InputValue::Float(1.5))?;
